...
class VideoNN:
    def improve_video(self, videofile, upd_videofile='untitled.avi', *args_realsr):#Function to improve the video
        ...
        for path in [fragments_path, upd_fragments_path]:
            if not os.path.exists(path):
                os.mkdir(path)
        if self.video_to_fragments(videofile, fragments_path) != 0:
        ...
        finish_returncode = self.use_realsr(fragments_path, upd_fragments_path, *args_realsr)

        if finish_returncode != 0:
            print('Error on upscaling frames')
            return -1

        if self.glue_frames(upd_fragments_path, upd_videofile_WOA) != 0:
            print('Error on glue frames')
            return -1
        if self.add_audio(upd_videofile_WOA, fragments_path + '/audio.mp3', upd_videofile) != 0:
            print('Error on adding audio')
            return -1

        return 0

    def use_realsr(self, input_path, output_path, *args_realsr, realsr_path='./realsr-ncnn-vulkan/realsr-ncnn-vulkan'):
        t1 = datetime.now()
    ...
    def video_to_fragments(self, path, output_path=None):
        # check paths
        if not os.path.exists(path):
    ...
    def glue_frames(self, src_path, videofile='untitled.avi', codec='h264', fps=30, *args_ffmpeg, photo_extenstion='png'):
    ...
    def add_audio(self, videofile, audio_path, new_name=None):
        if not new_name:
...
if __name__ == '__main__':
    ...
    video_nn.improve_video(*sys.argv[1:])